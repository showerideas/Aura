...
data class ExchangeAuditEntry(
    ...
    /**
     * Task 91 — Raw Groth16 ZK proof bytes (~192 bytes) generated by [GestureZkProver].
     *
     * Non-null only when [BuildConfig.ENABLE_ZK_AUDIT_PROOF] is true and the
     * gesture-biometric verification step ran during this exchange.  The proof
     * attests (without revealing the enrolled template) that the live gesture
     * cosine-similarity ≥ 0.85 threshold was met.
     *
     * Null for:
     *   - Legacy records (DB versions < 12, pre-v5.2 migration).
     *   - Exchanges where ZK audit is disabled or libgesturezk.so is absent.
     *   - Exchanges that failed before gesture verification was reached.
     */
    @ColumnInfo(name = "gesture_zk_proof")
    val gestureZkProof: ByteArray? = null
) {
    companion object {
